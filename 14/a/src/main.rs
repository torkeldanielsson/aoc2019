...
fn recursive_exchanger(rules: &Vec<Rule>, mut required: Vec<Produce>) -> i32 {
    ...
    for (i, req) in required.iter().enumerate() {
        if req.id != "ORE" {
            let mut spawned_req: Vec<Produce> = Vec::new();

            for (i_2, req_2) in required.iter().enumerate() {
                if i_2 != i {
                    spawned_req.push(req_2.clone());
                } else {
                    for rule in rules {
                        if rule.output.id == req.id {
                            let mut multiplier = 1;
            ...
            let mut clean_spawn_req: Vec<Produce> = Vec::new();
            for (req_prim_i, req_prim) in spawned_req.iter().enumerate() {
                let mut is_in_req = false;
                for r in &clean_spawn_req {
                    if r.id == req_prim.id {
                        is_in_req = true;
                    }
                }
                if !is_in_req {
                    ...
                    for (i, r) in spawned_req.iter().enumerate() {
                        if req_prim_i != i && r.id == req_prim.id {
                            sum_req.num += r.num;
                        }
                    }
                    clean_spawn_req.push(sum_req);
                }
            }

            // println!(
            //     "having: {:?}, spawning to test: {:?}",
            //     required, clean_spawn_req
            // );

            let spawn_res = recursive_exchanger(rules, clean_spawn_req);
            if spawn_res < lowest_res {
                lowest_res = spawn_res;
            }
        }
    }
    return lowest_res;
...
fn get_ore_count(rules: &Vec<Rule>) -> i32 {
    ...
    return recursive_exchanger(rules, required);
}
